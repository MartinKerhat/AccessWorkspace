...
func (s *Server) handleVaultAddPassphrase(w http.ResponseWriter, r *http.Request, user auth.User) {
	...
	s.auditVault(r, user, audit.EventVaultMethodAdded, "passphrase")
	writeJSON(w, http.StatusOK, map[string]string{"status": "ok"})
}
...
func (s *Server) handleVaultPasskeySetup(w http.ResponseWriter, r *http.Request, user auth.User) {
	...
	if err := s.authenticator.SetupVaultWithPasskey(r.Context(), user, requestSessionToken(r), input.CredentialID, input.PRFSalt, input.PRFSecret, input.Nickname); err != nil {
		writeError(w, err)
		return
	}
...
func (s *Server) handleVaultPasskeyAdd(w http.ResponseWriter, r *http.Request, user auth.User) {
	...
	if err := s.authenticator.AddVaultPasskey(r.Context(), user, input.CredentialID, input.PRFSalt, input.PRFSecret, input.Nickname); err != nil {
		writeError(w, err)
		return
	}
	s.auditVault(r, user, audit.EventVaultMethodAdded, "passkey")
	writeJSON(w, http.StatusOK, map[string]string{"status": "ok"})
}

func (s *Server) handleVaultMethodRemove(w http.ResponseWriter, r *http.Request, user auth.User) {
	var input struct {
	...
	if err := s.authenticator.RemoveVaultMethod(r.Context(), user, input.Method, input.Label); err != nil {
		writeError(w, err)
		return
	}
	s.auditVault(r, user, audit.EventVaultMethodRemoved, input.Method)
	writeJSON(w, http.StatusOK, map[string]string{"status": "ok"})
}

func (s *Server) handleVaultMethodRename(w http.ResponseWriter, r *http.Request, user auth.User) {
	var input struct {
	...
	if err := s.authenticator.RenameVaultPasskey(r.Context(), user, input.CredentialID, input.Nickname); err != nil {
		writeError(w, err)
		return
	}
	writeJSON(w, http.StatusOK, map[string]string{"status": "ok"})
